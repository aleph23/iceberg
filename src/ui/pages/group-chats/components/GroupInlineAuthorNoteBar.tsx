...
import { updateGroupSessionAuthorNote } from "../../../../core/storage/repo";
import { getPlatform } from "../../../../core/utils/platform";
import { cn } from "../../../design-tokens";
import { useI18n } from "../../../../core/i18n/context";

interface GroupInlineAuthorNoteBarProps {
  session: GroupSession | null | undefined;
  onSaved?: (session: GroupSession) => void;
}

export function GroupInlineAuthorNoteBar({ session, onSaved }: GroupInlineAuthorNoteBarProps) {
  ...
  const commit = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateGroupSessionAuthorNote(session.id, next || null);
      onSaved?.(updated);
    } catch (err) {
  ...
  return (
    <div className="group flex items-start gap-2.5 p-2">
      <div className="flex h-5 w-11 shrink-0 items-center justify-center text-fg/45" aria-hidden>
      ...
      {saving && (
        <span className="shrink-0 text-[10px] text-fg/35">{t("chats.authorNote.savingShort")}</span>
      )}
    </div>
  );
}
