...
export const jaMessages: DeepPartialMessageTree<LocaleMessages> = {
  ...
  tour: {
    ...
    gotIt: "了解",
    appShell: {
      chats: { title: "チャットはここにあります", body: "キャラクターとの1対1の会話がすべてここにあります。いつでも戻って続きから再開できます。" },
      groups: { title: "グループチャット", body: "複数のキャラクターを同じ部屋に集めて会話を観察したり、好きなタイミングで自分も参加できます。" },
      discover: { title: "新しいキャラクターを見つけよう", body: "コミュニティが共有したものを閲覧して、気に入ったキャラクターを追加しましょう。新しいお気に入りがワンタップで見つかります。" },
      library: { title: "あなたのライブラリ", body: "作成・保存したものがすべてここにあります：キャラクター、ペルソナ、プロンプトなど。あなたのコレクションです。" },
      settings: { title: "自分好みにカスタマイズ", body: "プロバイダーの変更、モデルの選択、見た目の調整。ほぼすべてが設定から変更できます。" },
      search: { title: "なんでも素早く検索", body: "特定のチャットやキャラクターを探していますか？ここからすべてを検索できます。掘り下げる必要はありません。" },
      create: { title: "最後に、作成しよう！", body: "インスピレーションが湧いたらプラスボタンをタップ。新しいキャラクター、ペルソナを作成したり、ゼロから始められます。" },
    },
    chatDetail: {
      chatTitle: { title: "チャットごとの設定", body: "上部のキャラクター名をタップすると、このチャット専用の設定を開きます。会話ごとに異なるペルソナ、レイアウト、モデルを選択できます。" },
      chatMemory: { title: "記憶していること", body: "脳のアイコンは、キャラクターがあなたとの会話から覚えていることを表示します。タップして記憶を確認、編集、またはクリアできます。" },
      chatSearch: { title: "あの一行を見つける", body: "この会話内だけを検索します。200メッセージ前のあの詳細を、永遠にスクロールせずに見つけられます。" },
      chatLorebook: { title: "ロアブックエントリ", body: "特定のキーワードが出現したときにプロンプトに挿入される追加事実、世界観構築、コンテキスト。キャラクターのカンニングペーパーです。" },
      chatPlus: { title: "ファイルを添付", body: "画像を追加したり、エクストラメニューを開いたりできます。添付したものは次のメッセージと一緒に送信されます。" },
      chatComposer: { title: "あなたのメッセージ、あなたの番", body: "ここに入力します。Enterで送信、Shift+Enterで改行。ヒント：チャット内のメッセージを長押しすると、編集、分岐、削除ができます。" },
      chatSend: { title: "1つのボタン、4つの役割", body: "送信ボタンは状況に応じて役割が変わります：" },
    },
    postFirstMessage: {
      chatRegenerate: { title: "不満？再生成", body: "更新アイコンをタップすると、キャラクターからまったく新しい返答が得られます。各再生成はバリアントとして保存され、後で見返せます。" },
      chatVariants: { title: "バリアント間をスワイプ", body: "再生成後、メッセージの下にバリアントカウンターが表示されます。メッセージバブルを左右にスワイプして、異なる返答を閲覧できます。" },
      chatLongPress: { title: "まだ隠れた機能があります", body: "メッセージを長押しすると、編集、コピー、分岐、ピン留め、削除、または会話の巻き戻しができます。デスクトップでは右クリックも使えます。" },
    },
    sendButton: {
      continue: { label: "続ける", desc: "入力が空です。ここをタップするとキャラクターに話し続けるよう促します。" },
      send: { label: "送信", desc: "テキストを入力したか、何かを添付しました。タップして送信します。" },
      sending: { label: "送信中", desc: "返答が送信中です。ボタンはロックされています。" },
      stop: { label: "停止", desc: "気が変わった場合、返答の途中でタップしてキャンセルします。" },
    },
  },
  ...
  samplerOrder: {
    ...
    stages: {
      penalties: { label: "ペナルティ", desc: "フィルタリング前に頻度ペナルティと存在ペナルティを適用。" },
      grammar: { label: "文法", desc: "ネイティブのチャットテンプレート文法がアクティブな場合にトークンを制約。" },
      topK: { label: "Top K", desc: "候補プールを最も強力なトークンに絞り込む。" },
      topP: { label: "Top P", desc: "残りの分布にNucleusフィルタリングを適用。" },
      minP: { label: "Min P", desc: "最小フロアを使用して低確率のテールトークンを除去。" },
      typical: { label: "Typical P", desc: "現在の分布で統計的に典型的なトークンを優先。" },
      temp: { label: "Temperature", desc: "選択前に最終分布を平坦化またはシャープ化。" },
    },
    presets: {
      default: { label: "デフォルト", hint: "Lettuce local" },
      unsloth: { label: "Unsloth", hint: "llama.cppスタイル" },
      focused: { label: "集中", hint: "タイトプルーニング" },
      creative: { label: "クリエイティブ", hint: "レイトフィルター" },
    },
  },
};
